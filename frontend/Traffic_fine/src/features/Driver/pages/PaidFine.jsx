...
import { 
    Menu, Hourglass, ListOrdered, 
    Coins, LayoutDashboard, FileText, 
    ChevronDown, LogOut, Info, Megaphone, User
} from 'lucide-react';
...
export default function PaidFine() {
    ...
    useEffect(() => {
        const fetchPaidFines = async () => {
            try {
                ...
                // 1. Get driver profile to get license number
                ...
                if (res.ok) {
                    ...
                    // Filter: License matches AND status is PAID
                    const filtered = allFines.filter(f => 
                        String(f.licenseId) === String(myLicenseNo) && 
                        (f.status === 'PAID' || f.paidDate)
                    );
                    
                    setPaidFines(filtered);
                }
            } catch (err) {
                console.error("Error loading paid fines:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const filteredFines = paidFines.filter(f => 
                            ...
                            <div className="p-4">
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full border-collapse">
                                        ...
                                        <tbody>
                                            {loading ? (
                                                <tr><td colSpan="7" className="py-8 text-center text-gray-500 italic">Loading paid fines...</td></tr>
                                                ...
                                                <tr key={index} className="border-b border-gray-200 text-sm text-gray-700 hover:bg-gray-50">
                                                    ...
                                                    <td className="py-3 px-3 text-green-700 font-medium">{fine.paidDate}</td>
                                                    <td className="py-3 px-3 font-bold">{(parseFloat(fine.totalAmount) || 0).toFixed(2)}</td>
                                                </tr>
                                            ))}
                                            {!loading && filteredFines.length === 0 && (
                                                <tr>
                                                    <td colSpan="7" className="py-12 text-center text-gray-500 bg-gray-50 flex flex-col items-center gap-2">
                                                        <Megaphone size={32} className="text-gray-300" />
                                                        <span>No paid fines recorded for your license.</span>
                                                    </td>
                                                </tr>
                                            )}
                                        </tbody>
                    ...
                    <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg overflow-hidden">
                        <div className="bg-[#17a2b8] text-white px-5 py-4 flex justify-between items-center">
                            <h3 className="text-lg font-bold flex items-center gap-2">
                                <Info size={20} /> Paid Fine Details
                            </h3>
                            ...
                            <table className="w-full text-sm border-collapse">
                                <tbody>
                                    {[
                                        ...
                                        { label: 'Paid Date', value: selectedFine.paidDate },
                                        ...
                                        { label: 'Status', value: selectedFine.status },
                                    ].map((row, i) => (
